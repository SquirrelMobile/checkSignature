...
#define TAG "fr.squirrel.checksignature"

using namespace v8;

static Persistent<Object> bindingCache;

static void Checksignature_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Checksignature.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::ChecksignatureBindings::lookupGeneratedInit(
...
static void Checksignature_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Checksignature_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Checksignature_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::ChecksignatureBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ChecksignatureBinding = {
	"fr.squirrel.checksignature",
	Checksignature_init,
	Checksignature_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_fr_squirrel_checksignature_ChecksignatureBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("fr.squirrel.checksignature", &ChecksignatureBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::ChecksignatureBindings::lookupGeneratedInit));
}
